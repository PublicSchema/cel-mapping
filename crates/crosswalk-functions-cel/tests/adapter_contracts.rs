use cel::Program;
use crosswalk_functions_cel::{FunctionRegistry, FunctionRequestContext};
use serde_json::json;
use std::{collections::BTreeSet, sync::Arc};

fn eval(expr: &str, request: FunctionRequestContext) -> serde_json::Value {
    let mut ctx = cel::Context::default();
    crosswalk_functions_cel::register_stdlib(&mut ctx, Arc::new(FunctionRegistry::new()));
    crosswalk_functions_cel::eval_ctx::set_eval_ctx(request);
...
fn regex_extract_no_match_is_empty_string_in_cel_adapter() {
    assert_eq!(
        crosswalk_functions::text::regex_extract("hello", r"\d+", 0).unwrap(),
        None
    );
    assert_eq!(
        eval(
...
fn helper_metadata_covers_registered_public_helpers_without_duplicates() {
    let metadata = crosswalk_functions_cel::helper_metadata();
