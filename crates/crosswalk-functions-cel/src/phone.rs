...
pub fn normalize_e164(input: &str, country: &Value) -> Result<String, String> {
    crosswalk_functions::phone::normalize_phone_e164(input, region_hint(country).as_deref())
        .map_err(|err| err.message)
}

pub fn is_valid(input: &str, country: &Value) -> bool {
    crosswalk_functions::phone::is_valid_phone(input, region_hint(country).as_deref())
}
...
pub fn country_calling_code(input: &str, country: &Value) -> Result<String, String> {
    crosswalk_functions::phone::country_calling_code(input, region_hint(country).as_deref())
        .map_err(|err| err.message)
}
