...
use crate::compiled::{CompiledCel, ErrorMode};
use crate::compiler::compile_expr;
use crate::errors::{
    CompileError, ErrorCode, ErrorSeverity, ExpressionPreviewResult, MappingError,
};
...
pub fn compile_publicschema_mapping(
    ...
    registry
        .merge_from_map(&extract_code_systems(&doc))
...
pub fn preview_publicschema_rule_expression(
    ...
    let rule: PropertyMappingYaml = match serde_json::from_value(mapping_rule.clone()) {
        ...
        Err(e) => {
            return ExpressionPreviewResult::from_parts(
                "".into(),
                None,
                vec![crate::errors::ExpressionIssue {
                    phase: crate::errors::ExpressionPhase::Evaluation,
    ...
    crate::evaluator::preview_cel_expression(expr, sample_record, ctx, limits, codes)
}
...
fn eval_rule_expression(
    ...
    let index_null = cel::Value::Null;

    let mut extras: Vec<(&str, cel::Value)> = Vec::with_capacity(2);
    match direction {
        PublicSchemaDirection::ToTarget => {
