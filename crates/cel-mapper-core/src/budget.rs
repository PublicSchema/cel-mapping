...
//! Installed for the duration of [`crate::runtime::MappingRuntime::evaluate`].
//!
...
thread_local! {
    static ACTIVE: RefCell<Option<Arc<SecurityLimits>>> = const { RefCell::new(None) };
}

/// RAII guard: installs limits for the current thread until dropped.
pub struct BudgetGuard {
    prev: Option<Arc<SecurityLimits>>,
}
...
fn current() -> Option<Arc<SecurityLimits>> {
