...
//! depends on `cel-evaluator` for expression execution and `mapping-functions`
//! for code-system data, but not on the `cel-mapper-core` facade.

use cel_evaluator::compile_expr;
use cel_evaluator::missing::MISSING_STR;
use cel_evaluator::output::cel_to_json;
use cel_evaluator::paths::{augment_json_with_paths, collect_missing_aware_injection_paths};
use cel_evaluator::SecurityLimits;
use cel_evaluator::{json_to_cel, run_program};
use cel_evaluator::{CompileError, CompiledCel, ErrorMode};
use cel_evaluator::{ErrorCode, ErrorSeverity, ExpressionPreviewResult, MappingError};
use mapping_functions::codes::{CodeEntry, CodeSystemDocument, CodeSystemRegistry};
use mapping_functions::FunctionError;
...
pub fn preview_publicschema_rule_expression(
    ...
    let rule: PropertyMappingYaml = match serde_json::from_value(mapping_rule.clone()) {
        ...
        Err(e) => {
            return ExpressionPreviewResult::from_parts(
                "".into(),
                None,
                vec![cel_evaluator::ExpressionIssue {
                    phase: cel_evaluator::ExpressionPhase::Evaluation,
    ...
    cel_evaluator::preview_cel_expression(expr, sample_record, ctx, limits, codes)
}
...
fn eval_rule_expression(
    ...
    let index_null = cel_evaluator::CelValue::Null;

    let mut extras: Vec<(&str, cel_evaluator::CelValue)> = Vec::with_capacity(2);
    match direction {
        PublicSchemaDirection::ToTarget => {
